...
static const char *tag = "Minitel"; // Device name(as seen by connected devices)
...
void (*bt_filenameCB)(const char*, uint8_t) = NULL;
void (*bt_fileSizeCB)(uint16_t) = NULL;
void (*bt_fileTypeCB)(const char*, uint8_t) = NULL;
void (*bt_dataCB)(const uint8_t*, uint16_t) = NULL;
void (*bt_dataEndCB)(void) = NULL;

void setFilenameCB(void (*fn)(const char*, uint8_t)){bt_filenameCB = fn;}
void setFileSizeCB(void (*fn)(uint16_t)){bt_fileSizeCB = fn;}
void setFileTypeCB(void (*fn)(const char*, uint8_t)){bt_fileTypeCB = fn;}
void setDataCB(void (*fn)(const uint8_t*, uint16_t)){bt_dataCB = fn;}
void setDataEndCB(void(*fn)(void)){bt_dataEndCB = fn;}
...
void __attribute__((weak)) obex_ftp(uint8_t *packet, uint16_t len)
{
    ...
    {
        log_d("send receive ack after 8 packets");
        pack_len = 13;
        UINT8_TO_STREAM(buf, pack_len / 256);
        UINT8_TO_STREAM(buf, pack_len % 256);
        UINT8_TO_STREAM(buf, H4_TYPE_ACL);
        ...
        case 0x02: // Obex PUT(Send file)
            ...
            for (int i = 14; i < len - 2;)
            {
                ...
                else if (packet[i] == 0x01)
                {
                    ...
                    log_d("Receiving File : ");
/*
                    for (int j = i + 3; j < head_len + i; j += 1){
...
*/
                    for (int j = i + 4; j < head_len + i; j += 2)
                        printf("%c", packet[j]);
                    if(bt_filenameCB != NULL) bt_filenameCB(&(packet[i + 3]), head_len);
                    i += head_len;
                    printf("\n");
                    fflush(stdout);
                }
                ...
                else if (packet[i] == 0xc3)
                {
                    ...
                    i += 5;
                    log_d("file is %i bytes long", file_len);
                    if(bt_fileSizeCB != NULL) bt_fileSizeCB(file_len);
                }
                ...
                else if (packet[i] == 0x48)
                {
                    ...
                    log_d("receiving data : packet is %i bytes long.", head_len);
...
//                    log_v("\n");
                    if(bt_dataCB != NULL) bt_dataCB(&(packet[i + 3]), head_len - 3);
                    i += head_len;
                }
                ...
                {
                    uint16_t head_len = packet[i + 1] * 256 + packet[i + 2];
                    log_d("file type info : %i bytes.", head_len);
//                    log_d("file type : %s", packet[i + 3]);
/*
                    for(int j = i + 3; j < head_len + i; j++){
//                        printf("%c", packet[j]);
                        printf("0x%02x ", packet[j]);
                    ...
                    printf("\n");
*/
                    if(bt_fileTypeCB != NULL) bt_fileTypeCB(&(packet[i + 3]), head_len);
                    i += head_len;
                }
                // Enable Single Response Mode request
                else if (packet[i] == 0x97)
                {
                    log_d("enable single response mode request");
                    send = 1;
                    pack_len = 18;
                    UINT8_TO_STREAM(buf, pack_len / 256);
                    UINT8_TO_STREAM(buf, pack_len % 256);
                ...
                else if (packet[i] == 0x49)
                {
                    ...
                    printf("File sum : %lld\n", file_sum);
                    fflush(stdout);
                    if(bt_dataEndCB != NULL) bt_dataEndCB();
                    // printf("End of file \n");
                }
                // another type of packet
                else
                {
                    log_d("unlisted packet of type 0x%02x", packet[i]);
                }
            }

            break;
        case 0xff:  // OBEW abort
            log_d("cmd : obex abort");
            break;
        default:
            log_d("cmd : 0x%02x", command);
...
void l2cap_cmd_handler(uint8_t *packet, uint16_t len)
{
    ...
    if (sig_channel == 0x01) // L2CAP packet (sig channel 0x0001 is reserved for L2CAP)
    {
        ...
        case L2CAP_CMD_INFO_REQ:
            log_d("cmd : information request");
            // Supported features request
            if (packet[13] == 0x02)
            {
                log_d("supported features req.");
                pack_len = 21;
            ...
            {
                log_d("fixed channel support req.");
                pack_len = 25;
                UINT8_TO_STREAM(buf, pack_len / 256);
                UINT8_TO_STREAM(buf, pack_len % 256);
                UINT8_TO_STREAM(buf, H4_TYPE_ACL);
        ...
        {
            log_d("cmd : Received connection request");
            // Accept connection
            pack_len = 21;
            UINT8_TO_STREAM(buf, pack_len / 256);
            UINT8_TO_STREAM(buf, pack_len % 256);
        ...
        case L2CAP_CMD_CONF_RES:
            log_d("cmd : configuration response");
...
/*
    ...
    if(ret){
        log_i("Bluetooth controller release classic bt memory failed: %s", esp_err_to_name(ret));
        return false;
    }
*/
    if (esp_bt_controller_get_status() == ESP_BT_CONTROLLER_STATUS_ENABLED)
    {
        log_d("BT already enabled");
        return true;
    }
    if (esp_bt_controller_get_status() == ESP_BT_CONTROLLER_STATUS_IDLE)
    {
        ...
        while (esp_bt_controller_get_status() == ESP_BT_CONTROLLER_STATUS_IDLE){
            log_d("waiting for BT controller init");
            log_i("status : %i", esp_bt_controller_get_status());
        }
    }
